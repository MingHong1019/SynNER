...
class FewShotDataset(data.Dataset):
    ...
    def __populate__(self, sentence, span, label,savelabeldic=False, mask_dataset=False):
        ...
        '''
        dataset = {'word': [], 'sentence': [], 'entity_types': [], 'entity_masks': [],'text_mask': [],\
        ...
        dataset['sentence'].append(current_word)
...
def collate_fn(data):
    support_sets= data
    batch_support = {'word': [], 'sentence':[], 'entity_types': [], 'entity_masks': [],'text_mask': [],'sentence_num': [],'label2tag':[], 'pos_span':[],'token_idx':[], 'non_nested_idx':[], 'data_idx':[]}
    ...
    no_merge = ['label2tag', 'pos_span', 'token_idx', 'non_nested_idx', 'sentence', 'data_idx']
    for i in range(len(batch_support['word'])):
        for key in batch_support:
            if key not in no_merge:
