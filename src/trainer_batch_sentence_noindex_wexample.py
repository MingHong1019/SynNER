...
class Trainer_batch_sentence_noindex_wexample(Trainer):
    ...
    def test(self, finetune_data_loader, target_test_loader, model_path):
        ...
        with torch.no_grad():
            ...
            result_file_name = f'result/{self.args.model_name}_{self.args.test_data}_{self.args.test_shots}shots_{self.args.llm_refine_batch_size}batchsize_{self.args.llm_refine_batch_threshold}threshold.jsonl'
            num_change_label = 0
    ...
    def batch_llm_revise_wo_sentence(self, revise_sentence, before_revise, pred_label, true_label, label2tag, logging):
        ...
        for key, value in before_revise.items():
            ...
            for i in value:
                ...
                prompt += f'I guess words span [ {i[0]} ] is a {i[1]} entity,\n'
        ...
        prompt += f'But I am not very sure. Please help me, If you think my guess is correct, please return my guess type. If you think my guess is wrong, return the entity type you believe it to be, or if you think the guessed words are not entities, please return O. Please output the list of results and separate by commas if there are multiple guesses, for example, PER, ORG. The number of output pairs must match the number of my guesses, no more, no less. Results must be be among {total_type}. ONLY return the results list, do NOT return any other information.'
        ...
        if len(result) == total_need_revice:
            ...
            for i in result:
            #     find  = 0
            #     for entity_type in target_type:
            #         if entity_type in i:
